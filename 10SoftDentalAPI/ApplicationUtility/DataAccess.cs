...
namespace ApplicationUtility
{
    public abstract class DataAccess : IDisposable
    {
        ...
        private SqlCommand BuildSqlCommand(string procedureName, IDataParameter[] parameters)
        {
            ...
            command.CommandTimeout = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["DB_CommandTimeOut"].ToString());
        ...
        public void SqlBulkInsert(DataTable dataTable, string TableName)
        {
            try
            {

                this._sqlConnection.Open();
                using (SqlBulkCopy sqlbc = new SqlBulkCopy(_sqlConnection))
                    {
                        sqlbc.DestinationTableName = TableName;
                        for (int i = 0; i < dataTable.Columns.Count; i++)
                        {
                            sqlbc.ColumnMappings.Add(dataTable.Columns[i].ColumnName.ToString(), dataTable.Columns[i].ColumnName.ToString());
                        }
